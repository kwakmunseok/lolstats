...
1. [프로젝트 개요](#1-프로젝트-개요)
2. [핵심 목표 (이 프로젝트로 기르고 싶은 역량)](#2-핵심-목표-이-프로젝트로-기르고-싶은-역량)
3. [기술 스택](#3-기술-스택)
4. [기능 명세 (MVP 우선순위)](#4-기능-명세-mvp-우선순위)
5. [화면 구성 및 사용자 플로우](#5-화면-구성-및-사용자-플로우)
6. [데이터 모델 상세 (ERD)](#6-데이터-모델-상세-erd)
7. [API 엔드포인트 명세](#7-api-엔드포인트-명세)
8. [예상 리스크 & 대응](#8-예상-리스크--대응)
9. [배포 계획](#9-배포-계획)
10. [예상 작업 일정](#10-예상-작업-일정)
11. [다음 액션](#11-다음-액션)
12. [Claude Code 활용 팁](#12-claude-code-활용-팁)
...
| 영역 | 기술 |
|---|---|
| 언어/프레임워크 | Java 17+, Spring Boot 4.1.x (2026-07-28 착수 시점 start.spring.io가 3.x 생성을 더 이상 지원하지 않아 4.1.x로 확정 — [PHASE1_PLAN.md](./PHASE1_PLAN.md) 결정 사항 참고) |
...
| 리스크 | 대응 방안 |
|---|---|
| Development Key 24시간 만료 | 개발 기간엔 매일 갱신으로 감수. **배포 시점(3주차)에 Personal API Key 신청** — 무료, 24h 만료 해결. 단 rate limit은 Dev와 동일하므로 아키텍처 제약은 그대로 (Production Key는 실사용자 있는 서비스용이라 신청 사유가 약함 — 신청 안 함) |
...
| 시점 | 작업 | 예상 시간 |
|---|---|---|
| 1주차 중 | 도메인 구매, EC2 인스턴스 생성만 해둠 | 1h |
| 3주차 | EC2 세팅(Docker, swap) + Nginx/HTTPS + 수동 배포 + **Personal API Key 신청**(24h 만료 없는 키로 상시 접속 가능하게) | 4~5h |
...
| 주차 | 기간 | 작업 | 주간 투입 |
|---|---|---|---|
| 1주차 | 07/28–08/03 | Phase 1 (~24h 진행) + 도메인/EC2 인스턴스만 생성(1h) | ≈ 25h |
| 2주차 | 08/04–08/10 | Phase 1 마무리(10~14h, ddragon 아이콘 포함) + Phase 2 착수: Bucket4j + 429 재시도부터(12~16h) | ≈ 26h |
| 3주차 | 08/11–08/17 | Phase 2 마무리: 수집 큐 + Redis(15~17h) + **수동 배포 + Personal Key 신청(4~5h)** + Phase 3 착수(4~6h) → **일요일: go/no-go 판단** | ≈ 26h |
...
1. Riot Developer Portal에서 Development API Key 발급 (개발은 Dev Key 제약 하에 진행 — Personal Key는 3주차 배포 시점에 신청, §9.6)
...
| Personal Key | 개인/소규모 프로젝트 | **3주차 배포 시점 신청** — 무료, 24h 만료 없음. rate limit은 Dev와 동일(제약 영구) |
| Production Key | 실사용자 있는 대규모 서비스 | **신청 안 함** — 동작하는 서비스/실사용 근거 필요, 개인 학습 프로젝트엔 신청 사유가 약함 |
