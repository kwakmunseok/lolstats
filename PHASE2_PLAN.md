...
- [x] 소환사 캐시 TTL(10분, PHASE1_PLAN.md §5) 재점검 결과: **그대로 유지**. 근거:
  1. **[전적 갱신] 버튼(Task 4)이 이미 탈출구를 제공** — TTL 안에서도 즉시 최신화하고 싶으면 갱신 버튼(60초 쿨다운)으로 우회 가능해졌으므로, 자동 캐시 TTL 자체를 짧게 유지할 필요성이 줄어듦
  2. **매치 수집은 TTL과 완전히 분리되어 있음** — `SummonerController`/`PageController` 둘 다 `planCollection`+`enqueue`를 캐시 히트/미스와 무관하게 **매 검색마다** 호출(§3 Task 3 코드 확인). 즉 TTL은 티어/레벨/LP 같은 "소환사 요약 정보"의 신선도에만 영향을 주고, 매치 히스토리가 채워지는 속도에는 영향 없음 — TTL을 더 짧게 잡아도 매치 수집이 더 빨라지지 않음
  3. **Rate limit 예산 보존 관점에서도 10분이 합리적** — TTL을 줄이면 같은 소환사를 반복 검색할 때마다 계정+소환사+랭크 정보(최대 3회) 재조회가 늘어나 전역 버킷(100회/2분)을 배경 수집 큐·다른 사용자 검색과 더 놓고 경쟁하게 됨
- [x] 결론을 아래 §5 결정 사항에 기록(신규 코드 없음 — 설계 재확인 태스크)

**완료 기준**: 없음(설계 노트) — 위 근거를 §5에 기록하는 것으로 완료.
...
| per-IP 제한 수치 | IP당 분당 10회 | 검색 1건이 동기적으로 최대 4회 Riot 호출을 유발(계정+소환사+랭크+매치ID목록) — 압도적 남용은 막되 정상 사용은 방해 안 함(§3 Task 6) |
| [전적 갱신] 쿨다운 | 60초 | 검색 캐시 TTL(10분)과 달리 명시적 사용자 액션이라 훨씬 짧게(§3 Task 4) |
| 소환사 캐시 TTL | 10분 유지(변경 없음) | Task 4(갱신 버튼)·Task 3(매치 수집이 TTL과 무관) 도입 후 재점검 — 그대로가 적절하다고 결론(§3 Task 7 근거 참고) |
| Redis 커맨드 타임아웃 | `spring.data.redis.timeout: 500ms` | 미설정 시 Lettuce 기본 60초가 적용돼 fail-open이 사실상 무의미해짐 — Task 6에서 라이브 검증 중 발견 |
